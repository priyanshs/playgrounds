/* 
Primitive Types--
Integers: u8, i8, u16, i16, u32, i32, u64, i64, u128, i128 
(number of bits they take in memory)
Floats: f32, f64
Boolean (bool)
Characters (char)
Tuples
Arrays
*/

// // Rust is a statically typed language, which means that it must know the types of all variables at compile time, however, the compiler can usually infer what type we want to use based on the value and how we use it.

pub fn run()
{
    // Defaults 
    let x = 1; // i32 
    let y = 2.5; // f64

    //Explicitly typed
    let z: i128 = 821783712897389017; // i128

    // Find Max i32 and i128
    println!("Max i32: {}", std::i32::MAX);
    println!("Max i128:{}", std::i128::MAX);

    // Boolean
    let is_active = true; // explicit asign 
    let is_greater = 10 > 5; // Get val from expression 

    // Charaters
    let chr = 'c';

    println!("{:?}", (x, y, z, is_active, is_greater,chr));

}